...
QDM assumes a mapping within quantiles that the Delta Method doesn't.  For extreme precipitation, in the high percentiles, this can be very important, because this rainfall often behaves differently when compared to typical, or median precipitation.  

## Question 6

### 6A

Vandal et al. (2017) assumes that geography/topography, as well as the method that DeepSD learns to map data from low resolution to high resolution, are stationary.  It allows for the climate itself to change, as long as it does not change in such a way that this mapping is no longer valid. 

### 6B

Weather regime frequencies and within-regime precipitation intensities both may change in a non-stationary climate.  Which of these changes, or if both change, and by how much, may have impacts for climate damage mitigation and our calculations of climate risk.  The physical understanding that motivates this approach is that how many storms there are does not neccessarily tell us how intense those storms are.  Or, put more broadly, the category of weather (rain, heat, cold, etc.) does not necessarily tell us how intense the weather is within that category. 


### 6C

The deep learning approach has the advantage of being able to simulate a variety of climate scenarios.  As long as the assumption tahat the mapping learned by DeepSD does not need to be changed in a non-stationary climate holds, then we can project risk in a variety of different climate change scenarios.  One disadvantage is that neural networks often have difficulty extrapolating beyond training conditions, so if, as a result, our assumption does not hold, then we will make very poor risk assesments. 