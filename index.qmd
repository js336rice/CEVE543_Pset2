...
Plot synthetic states: need to modify this still 
...
let
    ...
    #Load in grid 
    nlons = length(pca_data.coordinates.lon)
    nlats = length(pca_data.coordinates.lat)

    #Synthetic spatial anomaly maps 
    #Replace ψ_ndjf with this 
    ψ_synth = zeros(nlons, nlats, length(random_states))
    ...
    #Same loop as in original code, but added in our data 
    for k in 1:n_states
        cluster_days = findall(random_states .== k)
        if length(cluster_days) > 0
            cluster_data = ψ_synth[:, :, cluster_days]
            synthetic_composites[:, :, k] =
                mean(cluster_data, dims=3)[:, :, 1]
        end
    end

    # Color range based on composites
    crange = maximum(abs.(extrema(synthetic_composites)))
    crange = (-crange, crange)
    ...
    for k in 1:n_states
        ...
        ax = create_geoaxis(fig2, (row, col);
            title="State $k: Synthetic Composite (n=$(sum(all_states .== k)))")
        heatmap!(ax, pca_data.coordinates.lon, pca_data.coordinates.lat,
            synthetic_composites[:, :, k]; colormap=cmap_diverging, colorrange=crange)
        add_boundaries!(ax)
    end
...
The state frequencies are certainly different for the synthetic data, but not in a pattern that's easy to name or take an obvious conclusion from with the naked eye.  

The synthetic transition matrix is very similar, which is to be expected. 

The states themselsves are similar, but their ranks are very different. 
...
You would plug the prior history of weather into you HMM, and attempt to use it to preduct the most likely state of the next day.  If you wanted to do a multi-day forecast, you would add the forecasted day to your data, and then re-run the HMM to attempt to preduct the next day, and repeat as necassary for as far as you want your forecast to go. 

To validate if HMM states relate to rainfall patterns, I'd look at what days the HMM groups into which patterns, and either visually inspect the data, or us some kind of error bounding algorithm, like MSE or something more suited to 2D data like this, like Digital Image Correlation, to quantify how similar the days grouped in to each state are to that state. 

For extreme rainfall, there is a fundamental limitation in that it is, by definition, rare.  HMM's predict what is most likely to happen, not what will happen.  An HMM is not a tool suited to predicting unlikely events such as extreme rainfall. 
